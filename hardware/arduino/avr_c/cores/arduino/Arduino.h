...
#ifdef BORDER_ROUTER
#undef UART0_UDRE_vect
#undef UART1_UDRE_vect
#undef USART1_RX_vect
#undef USART0_RX_vect
#endif

#include <util/delay.h>
#define ILOOPPROCESS_BEGIN() 	PROCESS(iloop_process, "loop process"); \
		PROCESS_THREAD(iloop_process, ev, data) \
		{										\
	static struct etimer iduinotimer;	\
	PROCESS_BEGIN();					\
	while (1)							\
	{									\

#define ILOOPPROCESS_START()   process_start(&iloop_process,NULL)
#define ILOOPPROCESSP_END()			} 	\
		PROCESS_END(); \
		}				\

#define IDELAY(a)	if (a>=200) 							\
		{									\
	etimer_set(&iduinotimer, ((uint32_t)a*CLOCK_CONF_SECOND)/1000); \
